import type { Address, GoogleSolarData } from '../types';
import { GOOGLE_API_KEY, IS_API_KEY_VALID } from '../config/googleApiConfig';

// =================================================================================
//  Session-Based Cache for Solar API
// =================================================================================
// To enhance performance and reduce API costs, we use a simple in-memory cache.
//
// - Key: A string combination of "latitude,longitude".
// - Value: The full JSON response from the GoogleSolarData API.
//
// This `Map` object persists for the duration of the user's browser session.
// If the user analyzes the same address twice without reloading the page, the
// data is served directly from this cache, avoiding a redundant API call.
// The cache is cleared on a full page refresh.
// =================================================================================
...
export const getSolarData = async (address: Address): Promise<GoogleSolarData> => {
    ...
    if (!IS_API_KEY_VALID) {
        throw new Error("API key is not configured. Please set the API_KEY in config/googleApiConfig.ts");
    }
    ...
    try {
        ...
        console.log("⬇️ Fetched new solar data. Caching result for:", cacheKey);
